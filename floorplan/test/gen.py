...
def gen_blocks(num_of_blocks: int) -> List[Block]:
    ...
    range of [3, 12].
    """
    RANGE: Final[Tuple[int, int]] = (3, 12)
    blocks: List[Block] = []
    for _ in range(num_of_blocks):
        width = randint(*RANGE)
        height = randint(*RANGE)
...
def gen_area_constraint() -> Tuple[float, float]:
    ...
    """
    # We generate them as integers and later divide to limit the precision of fractional parts.
...
The format is as the following:
    ...
    <block name> <block width> <block height>+
    ```
Ensure that the number of blocks is greater than 1 to make the floorplanning meaningful.
""",
    )
